...
static void flow_up(heap *self) {
    ...
    while (true) {
        ...
        void *parent_item = (void *)((uint8_t *)self->storage +
                                     (parent_item_index * self->item_size));
        void *current_item = (void *)((uint8_t *)self->storage +
                                      (current_item_index * self->item_size));
        int comparing_result = self->comparator(current_item, parent_item);
        if (((comparing_result > 0) && (self->max_heap)) ||
            ((comparing_result < 0) && (!self->max_heap)))
        // ((current_item > parent_item) && (max_heap)) || ((current_item <
        // parent_item) && (!max_heap))
        {
...
static void flow_down(heap *self) {
    ...
    while (true) {
        ...
        void *current_item = (void *)((uint8_t *)self->storage +
                                      (current_item_index * self->item_size));
        void *left =
            (void *)((uint8_t *)self->storage + (left_index * self->item_size));
        // left is unsafe ptr; use only if left_index != -1
        void *right = (void *)((uint8_t *)self->storage +
                               (right_index * self->item_size));
        // right is unsafe ptr; use only if right_index != -1
        if ((left_index != -1) && (right_index != -1)) {
            int lr_comparing_result = self->comparator(left, right);
            if (((lr_comparing_result > 0) && self->max_heap) ||
                ((lr_comparing_result < 0) && !self->max_heap)) {
                right_index = -1;
            } else {
                left_index = -1;
            }
...
bool heap_insert(heap *self, void *item) {
    ...
    void *dest = (void *)((uint8_t *)self->storage +
                          (self->item_size * self->items_count));
    if (dest != item) {
...
bool heap_pop(heap *self, void *item_receiver) {
    ...
    self->items_count--;
    if (self->items_count) {
        memmove(self->storage,
                (void *)((uint8_t *)self->storage +
                         (self->item_size * self->items_count)),
                self->item_size);
        flow_down(self);
    }
...
void heap_inplace_heapify(heap *dest,
                          void *array,
                          uint32_t size,
                          uint32_t item_size,
                          int (*comparator)(const void *, const void *),
...
void heap_inplace_sort(void *array,
                       uint32_t size,
                       uint32_t item_size,
                       int (*comparator)(const void *, const void *),
                       bool ascending) {
    heap sorter;
    heap_inplace_heapify(&sorter,
                         array,
                         size,
                         item_size,
                         comparator,
    ...
    for (int32_t i = size - 1; i >= 0; i--) {
        ...
        memcpy((void *)((uint8_t *)array + (item_size * i)),
               tmp_storage,
               item_size);
    }
}
