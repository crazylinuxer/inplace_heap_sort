...
bool check_sorted_sequence(void *storage,
    ...
    for (uint32_t i = 0; i < items_count - 1; i++) {
        int comparing_result =
            comparator((void *)((uint8_t *)storage + (item_size * i)),
                       (void *)((uint8_t *)storage + (item_size * (i + 1))));
        if (((comparing_result < 0) && !ascending) ||
            ((comparing_result > 0) && ascending)) {
            return false;
        }
    }
    return true;
...
int main() {
    ...
    for (uint32_t i = 0; i < TESTS_COUNT; i++) {
        ...
        heap_inplace_sort(data,
                          sizeof(data) / sizeof(uint32_t),
                          sizeof(uint32_t),
                          integer_comparator,
                          true);
        int64_t microseconds_elapsed =
            get_current_microsecond() - start_microsecond;
        if (!check_sorted_sequence(data,
                                   sizeof(data) / sizeof(uint32_t),
                                   sizeof(uint32_t),
                                   integer_comparator,
                                   true)) {
